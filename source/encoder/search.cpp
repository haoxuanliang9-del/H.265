...
void Search::checkIntra(Mode& intraMode, const CUGeom& cuGeom, PartSize partSize)
{
    ...
    cu.setPartSizeSubParts(partSize); // 设置CU分区大小
    cu.setPredModeSubParts(MODE_INTRA); // 设置为帧内预测模式

    uint32_t tuDepthRange[2];
    cu.getIntraTUQtDepthRange(tuDepthRange, 0); // 获取TU四叉树深度范围[min,max]

    intraMode.initCosts(); // 初始化各项代价累加器
    intraMode.lumaDistortion += estIntraPredQT(intraMode, cuGeom, tuDepthRange); // 亮度帧内预测,返回最优亮度失真
    if (m_csp != X265_CSP_I400) // 非单色度,需做色度帧内预测
    {
        intraMode.chromaDistortion += estIntraPredChromaQT(intraMode, cuGeom); // 色度帧内预测
        intraMode.distortion += intraMode.lumaDistortion + intraMode.chromaDistortion;
    }
    else
        intraMode.distortion += intraMode.lumaDistortion;
    cu.m_distortion[0] = intraMode.distortion; // 记录CU总失真
    m_entropyCoder.resetBits(); // 熵编码器比特计数清零,开始估算编码比特
    if (m_slice->m_pps->bTransquantBypassEnabled)
        m_entropyCoder.codeCUTransquantBypassFlag(cu.m_tqBypass[0]);

    int skipFlagBits = 0;
    if (!m_slice->isIntra()) // P/B帧需编码skip标志与预测模式标志
    {
        m_entropyCoder.codeSkipFlag(cu, 0);
    ...
    m_entropyCoder.codePartSize(cu, 0, cuGeom.depth); // 编码分区大小
    m_entropyCoder.codePredInfo(cu, 0); // 编码帧内预测信息(模式、角度等)
    intraMode.mvBits = m_entropyCoder.getNumberOfWrittenBits() - skipFlagBits; // 预测信息比特

    bool bCodeDQP = m_slice->m_pps->bUseDQP;
    m_entropyCoder.codeCoeff(cu, 0, bCodeDQP, tuDepthRange); // 编码残差系数
    m_entropyCoder.store(intraMode.contexts); // 保存熵编码上下文供后续RDO复用
    intraMode.totalBits = m_entropyCoder.getNumberOfWrittenBits(); // 总比特
    intraMode.coeffBits = intraMode.totalBits - intraMode.mvBits - skipFlagBits; // 系数比特
    const Yuv* fencYuv = intraMode.fencYuv;
    if (m_rdCost.m_psyRd) // Psy-RD:计算心理视觉能量
    ...
    else if(m_rdCost.m_ssimRd) // SSIM-RD:计算SSIM能量
    ...
    intraMode.resEnergy = primitives.cu[cuGeom.log2CUSize - 2].sse_pp(intraMode.fencYuv->m_buf[0], intraMode.fencYuv->m_size, intraMode.predYuv.m_buf[0], intraMode.predYuv.m_size); // 残差能量(预测SSE)

    updateModeCost(intraMode); // 计算最终RD代价 distortion + lambda*bits
    checkDQP(intraMode, cuGeom); // 检查是否需要调整QP
}
...
{
    ProfileCUScope(interMode.cu, motionEstimationElapsedTime, countMotionEstimate); // 运动估计耗时统计

    CUData& cu = interMode.cu;
    Yuv* predYuv = &interMode.predYuv;

    // 12 mv candidates including lowresMV
    MV mvc[(MD_ABOVE_LEFT + 1) * 2 + 2]; // MV候选数组(含lowresMV)

    const Slice *slice = m_slice;
    int numPart     = cu.getNumPartInter(0); // 当前CU的PU个数
    int numPredDir  = slice->isInterP() ? 1 : 2; // 预测方向数:P帧1个(L0),B帧2个(L0/L1)
    const int* numRefIdx = slice->m_numRefIdx; // 各方向参考帧数
    uint32_t lastMode = 0;
    int      totalmebits = 0;
    MV       mvzero(0, 0);
    ...
    bool useAsMVP = false;
    for (int puIdx = 0; puIdx < numPart; puIdx++) // 遍历CU内所有PU
    {
        MotionData* bestME = interMode.bestME[puIdx]; // 各方向最优运动数据(L0/L1)
        PredictionUnit pu(cu, cuGeom, puIdx);
        m_me.setSourcePU(*interMode.fencYuv, pu.ctuAddr, pu.cuAbsPartIdx, pu.puAbsPartIdx, pu.width, pu.height, m_param->searchMethod, m_param->subpelRefine, bChromaMC); // 配置运动估计源PU
        useAsMVP = false;
        x265_analysis_inter_data* interDataCTU = NULL;
        int cuIdx;
        cuIdx = (interMode.cu.m_cuAddr * m_param->num4x4Partitions) + cuGeom.absPartIdx;
        if (m_param->analysisLoadReuseLevel == 10 && m_param->interRefine > 1) // 旁路:analysis复用level10,主线可跳过
        {
        ...
        uint32_t mrgCost = numPart == 1 ? MAX_UINT : mergeEstimation(cu, cuGeom, pu, puIdx, merge); // merge候选代价(2Nx2N单PU时跳过)
        bestME[0].cost = MAX_UINT; // L0最优代价初始化
        bestME[1].cost = MAX_UINT; // L1最优代价初始化

        getBlkBits((PartSize)cu.m_partSize[0], slice->isInterP(), puIdx, lastMode, m_listSelBits); // 计算列表选择比特
        bool bDoUnidir = true; // 是否走单向预测主分支

        cu.getNeighbourMV(puIdx, pu.puAbsPartIdx, interMode.interNeighbours); // 获取空域/时域邻居MV,供AMVP/merge使用
        /* Uni-directional prediction */
        if ((m_param->analysisLoadReuseLevel > 1 && m_param->analysisLoadReuseLevel != 10) // 旁路:analysis复用或多pass精化,主线可跳过
            || (m_param->analysisMultiPassRefine && m_param->rc.bStatRead) || (m_param->bAnalysisType == AVC_INFO) || (useAsMVP))
        {
            for (int list = 0; list < numPredDir; list++) // 遍历L0/L1
            {
                ...
                if (useAsMVP)
                    ...
                    ref = bestME[list].ref; // 使用单向搜索阶段确定的最优参考帧
                if (ref < 0)
                {
                    continue;
                }
                uint32_t bits = m_listSelBits[list] + MVP_IDX_BITS; // 列表选择+MVP索引比特
                bits += getTUBits(ref, numRefIdx[list]); // 参考帧索引比特

                int numMvc = cu.getPMV(interMode.interNeighbours, list, ref, interMode.amvpCand[list][ref], mvc); // 生成AMVP候选并收集MV候选(供me剪枝)
                const MV* amvp = interMode.amvpCand[list][ref]; // AMVP候选(最多2个)
                int mvpIdx = selectMVP(cu, pu, amvp, list, ref); // 预选MVP索引
                ...
                if (useAsMVP)
                {
                    ...
                    mvp = amvp[mvpIdx];
                if (m_param->searchMethod == X265_SEA) // 旁路:SEA积分图设置,主线可跳过
                {
                    int puX = puIdx & 1;
                    int puY = puIdx >> 1;
                ...
                setSearchRange(cu, mvp, m_param->searchRange, mvmin, mvmax); // 以MVP为中心设置搜索范围
                MV mvpIn = mvp;
                int satdCost;
                if (m_param->analysisMultiPassRefine && m_param->rc.bStatRead && mvpIdx == bestME[list].mvpIdx)
                    mvpIn = bestME[list].mv;
                if (useAsMVP && m_param->mvRefine > 1) // 旁路:useAsMVP下的MVP精化,主线可跳过
                {
                    MV bestmv, mvpSel[3];
                    int mvpIdxSel[3];
                ...
                {
                    satdCost = m_me.motionEstimate(&slice->m_mref[list][ref], mvmin, mvmax, mvpIn, numMvc, mvc, m_param->searchRange, outmv, m_param->maxSlices, // 主线:调用运动估计,返回SATD代价并输出MV
                        m_param->bSourceReferenceEstimation ? m_slice->m_refFrameList[list][ref]->m_fencPic->getLumaAddr(0) : 0);
                }

                /* Get total cost of partition, but only include MV bit cost once */
                bits += m_me.bitcost(outmv); // MVD比特
                uint32_t mvCost = m_me.mvcost(outmv); // MVD代价(satdCost已含,需扣除避免重复)
                uint32_t cost = (satdCost - mvCost) + m_rdCost.getCost(bits); // PU总代价 = satd - mvCost + lambda*bits
                /* Refine MVP selection, updates: mvpIdx, bits, cost */
                if (!(m_param->analysisMultiPassRefine || useAsMVP))
                    mvp = checkBestMVP(amvp, outmv, mvpIdx, bits, cost); // 精化MVP(选更省比特的候选)
                else
                {
                ...
                if (cost < bestME[list].cost) // 更新该方向最优运动数据
                {
                    bestME[list].mv = outmv;
                    bestME[list].mvp = mvp;
                    bestME[list].mvpIdx = mvpIdx;
                ...
                bDoUnidir = false; // 已通过复用分支处理,跳过单向主分支
            }            
        }
        else if (m_param->bDistributeMotionEstimation) // 旁路:多线程分布式运动估计PME,主线可跳过
        {
        ...
        if (bDoUnidir) // 主线:单向预测,遍历所有list的所有参考帧做运动估计
        {
            interMode.bestME[puIdx][0].ref = interMode.bestME[puIdx][1].ref = -1;
            uint32_t refMask = refMasks[puIdx] ? refMasks[puIdx] : (uint32_t)-1; // 参考帧掩码(按位启用)

            for (int list = 0; list < numPredDir; list++) // 遍历L0/L1
            {
                for (int ref = 0; ref < numRefIdx[list]; ref++) // 遍历该方向所有参考帧
                {
                    ...
                    uint32_t bits = m_listSelBits[list] + MVP_IDX_BITS; // 列表+MVP索引比特
                    bits += getTUBits(ref, numRefIdx[list]); // 参考帧索引比特

                    int numMvc = cu.getPMV(interMode.interNeighbours, list, ref, interMode.amvpCand[list][ref], mvc); // 生成AMVP候选与MV候选

                    const MV* amvp = interMode.amvpCand[list][ref]; // AMVP候选
                    int mvpIdx = selectMVP(cu, pu, amvp, list, ref); // 预选MVP
                    MV mvmin, mvmax, outmv, mvp = amvp[mvpIdx], mvp_lowres;
                    bool bLowresMVP = false;

                    if (!m_param->analysisSave && !m_param->analysisLoad) /* Prevents load/save outputs from diverging when lowresMV is not available */
                    {
                        MV lmv = getLowresMV(cu, pu, list, ref); // 取低分辨率MV作为额外候选
                        if (lmv.notZero())
                            mvc[numMvc++] = lmv;
                        if (m_param->bEnableHME)
                            mvp_lowres = lmv;
                    ...
                    setSearchRange(cu, mvp, m_param->searchRange, mvmin, mvmax); // 以MVP为中心设置搜索范围
                    int satdCost = m_me.motionEstimate(&slice->m_mref[list][ref], mvmin, mvmax, mvp, numMvc, mvc, m_param->searchRange, outmv, m_param->maxSlices, // 调用运动估计,返回SATD代价
                      m_param->bSourceReferenceEstimation ? m_slice->m_refFrameList[list][ref]->m_fencPic->getLumaAddr(0) : 0);

                    if (m_param->bEnableHME && mvp_lowres.notZero() && mvp_lowres != mvp) // HME:用低分辨率MVP再做一次搜索取优
                    {
                        MV outmv_lowres;
                    ...
                    /* Get total cost of partition, but only include MV bit cost once */
                    bits += m_me.bitcost(outmv); // MVD比特
                    uint32_t mvCost = m_me.mvcost(outmv); // MVD代价
                    uint32_t cost = (satdCost - mvCost) + m_rdCost.getCost(bits); // PU总代价
                    /* Update LowresMVP to best AMVP cand*/
                    if (bLowresMVP)
                        updateMVP(amvp[mvpIdx], outmv, bits, cost, mvp_lowres);

                    /* Refine MVP selection, updates: mvpIdx, bits, cost */
                    mvp = checkBestMVP(amvp, outmv, mvpIdx, bits, cost); // 精化MVP

                    if (cost < bestME[list].cost) // 更新该方向最优
                    {
                        bestME[list].mv      = outmv;
                        bestME[list].mvp     = mvp;
                        bestME[list].mvpIdx  = mvpIdx;
        ...
        if (slice->isInterB() && !cu.isBipredRestriction() &&  /* biprediction is possible for this PU */ // B帧且允许双向预测
            cu.m_partSize[pu.puAbsPartIdx] != SIZE_2Nx2N &&    /* 2Nx2N biprediction is handled elsewhere */
            bestME[0].cost != MAX_UINT && bestME[1].cost != MAX_UINT) // L0/L1均有有效结果
        {
            bidir[0] = bestME[0]; // 取L0最优
            bidir[1] = bestME[1]; // 取L1最优

            int satdCost;

            if (m_me.bChromaSATD) // 旁路:色度SATD评估双向,主线可跳过
            {
            ...
            else
            {
                ...
                predInterLumaPixel(pu, bidirYuv[0], *refPic0, bestME[0].mv); // MC:取L0参考子像素
                predInterLumaPixel(pu, bidirYuv[1], *refPic1, bestME[1].mv); // MC:取L1参考子像素
                primitives.pu[m_me.partEnum].pixelavg_pp[(tmpPredYuv.m_size % 64 == 0) && (bidirYuv[0].m_size % 64 == 0) && (bidirYuv[1].m_size % 64 == 0)](tmpPredYuv.m_buf[0], tmpPredYuv.m_size, bidirYuv[0].getLumaAddr(pu.puAbsPartIdx), bidirYuv[0].m_size, // 两路MC结果加权平均(1:1)得双向预测
                                                                                                 bidirYuv[1].getLumaAddr(pu.puAbsPartIdx), bidirYuv[1].m_size, 32);
                satdCost = m_me.bufSATD(tmpPredYuv.m_buf[0], tmpPredYuv.m_size); // 对双向预测块算SATD
            }

            bidirBits = bestME[0].bits + bestME[1].bits + m_listSelBits[2] - (m_listSelBits[0] + m_listSelBits[1]); // 双向比特=L0+L1+双向列表标志
            bidirCost = satdCost + m_rdCost.getCost(bidirBits); // 双向总代价
        ...
        if (mrgCost < bidirCost && mrgCost < bestME[0].cost && mrgCost < bestME[1].cost) // merge最优:写merge模式
        {
        ...
        else if (bidirCost < bestME[0].cost && bidirCost < bestME[1].cost) // 双向最优:写L0+L1
        {
            lastMode = 2;
        ...
        else if (bestME[0].cost <= bestME[1].cost) // L0单向最优
        {
            lastMode = 0;
        ...
        motionCompensation(cu, pu, *predYuv, true, bChromaMC); // 用最终MV做运动补偿生成预测块
    }
    interMode.sa8dBits += totalmebits; // 累加模式决策比特
}
...
void Search::estimateResidualQT(Mode& mode, const CUGeom& cuGeom, uint32_t absPartIdx, uint32_t tuDepth, ShortYuv& resiYuv, Cost& outCosts, const uint32_t depthRange[2], int32_t splitMore) // 残差四叉树(TU)编码代价估算
{
    CUData& cu = mode.cu;
    uint32_t depth = cuGeom.depth + tuDepth; // 当前TU总深度
    uint32_t log2TrSize = cuGeom.log2CUSize - tuDepth; // 当前变换块尺寸log2
    bool bEnableRDOQ = !!m_param->rdoqLevel; // 是否启用RDOQ

    bool bCheckSplit = log2TrSize > depthRange[0]; // 是否检查TU分裂(更小尺寸)
    bool bCheckFull = log2TrSize <= depthRange[1]; // 是否检查整块(当前尺寸)
    bool bSaveTUData = false, bLoadTUData = false;
    uint32_t idx = 0;

    if ((m_limitTU & X265_TU_LIMIT_BFS) && splitMore >= 0) // 旁路:TU限制BFS复用,主线可跳过
    {
    ...
    else if (m_limitTU & X265_TU_LIMIT_DFS || m_limitTU & X265_TU_LIMIT_NEIGH) // 旁路:TU深度限制DFS/邻居,主线可跳过
    {
    ...
    bool bSplitPresentFlag = bCheckSplit && bCheckFull; // 是否编码split flag

    if (cu.m_partSize[0] != SIZE_2Nx2N && !tuDepth && bCheckSplit) // 非对称分区根TU不检查整块
        bCheckFull = false;

    X265_CHECK(bCheckFull || bCheckSplit, "check-full or check-split must be set\n");

    uint32_t log2TrSizeC = log2TrSize - m_hChromaShift; // 色度变换块尺寸
    uint32_t codeChroma = (m_csp != X265_CSP_I400 && m_frame->m_fencPic->m_picCsp != X265_CSP_I400) ? 1 : 0; // 是否编码色度
    uint32_t tuDepthC = tuDepth;
    if (log2TrSizeC < 2) // 色度块过小,提升尺寸并按4:2:0规则避免重复编码
    {
        X265_CHECK(log2TrSize == 2 && m_csp != X265_CSP_I444 && tuDepth, "invalid tuDepth\n");
        log2TrSizeC = 2;
        tuDepthC--;
    ...
    if (bCheckFull) // 整块变换分支
    {
        ...
        uint32_t coeffOffsetY = absPartIdx << (LOG2_UNIT_SIZE * 2);
        coeff_t* coeffCurY = m_rqt[qtLayer].coeffRQT[0] + coeffOffsetY; // 当前亮度系数缓冲
        ...
        if (bEnableRDOQ)
            m_entropyCoder.estBit(m_entropyCoder.m_estBitsSbac, log2TrSize, true); // RDOQ估算比特

        const pixel* fenc = fencYuv->getLumaAddr(absPartIdx);
        int16_t* resi = resiYuv.getLumaAddr(absPartIdx);
        numSig[TEXT_LUMA][0] = m_quant.transformNxN(cu, fenc, fencYuv->m_size, resi, resiYuv.m_size, coeffCurY, log2TrSize, TEXT_LUMA, absPartIdx, false); // 正变换+量化,返回非零系数个数
        cbfFlag[TEXT_LUMA][0] = !!numSig[TEXT_LUMA][0]; // CBF标志

        m_entropyCoder.resetBits(); // 开始估算编码比特

        if (bSplitPresentFlag && log2TrSize > depthRange[0])
            m_entropyCoder.codeTransformSubdivFlag(0, 5 - log2TrSize); // 编码不分裂标志

        if (cbfFlag[TEXT_LUMA][0])
            m_entropyCoder.codeCoeffNxN(cu, coeffCurY, absPartIdx, log2TrSize, TEXT_LUMA); // 编码非零系数
        singleBits[TEXT_LUMA][0] = m_entropyCoder.getNumberOfWrittenBits(); // 亮度系数比特

        X265_CHECK(log2TrSize <= 5, "log2TrSize is too large\n");

        //Assuming zero residual 
        sse_t zeroDistY = primitives.cu[partSize].sse_pp(fenc, fencYuv->m_size, mode.predYuv.getLumaAddr(absPartIdx), mode.predYuv.m_size); // 零残差失真(预测vs原始)
        ...
        if (cbfFlag[TEXT_LUMA][0]) // CBF非零:反量化反变换求重建
        {
            m_quant.invtransformNxN(cu, curResiY, strideResiY, coeffCurY, log2TrSize, TEXT_LUMA, false, false, numSig[TEXT_LUMA][0]); //this is for inter mode only // 反量化反变换得残差
            ...
            primitives.cu[partSize].add_ps[bufferAlignCheck](curReconY, strideReconY, mode.predYuv.getLumaAddr(absPartIdx), curResiY, mode.predYuv.m_size, strideResiY); // 重建 = 预测 + 残差

            const sse_t nonZeroDistY = primitives.cu[partSize].sse_pp(fenc, fencYuv->m_size, curReconY, strideReconY); // 非零残差失真(重建vs原始)
            ...
            else if(m_rdCost.m_ssimRd) // SSIM-RD代价
            {
                ...
                singleCostY = m_rdCost.calcRdCost(nonZeroDistY, nzCbfBitsY + singleBits[TEXT_LUMA][0]); // 普通RD代价

            if (cu.m_tqBypass[0]) // transquant bypass:直接用非零结果
            {
            ...
            else
            {
                ...
                uint64_t nullCostY = estimateNullCbfCost(zeroDistY, zeroEnergyY, tuDepth, TEXT_LUMA); // 零CBF代价(不编码系数)

                if (nullCostY < singleCostY) // 零CBF更优:清空CBF与比特
                {
