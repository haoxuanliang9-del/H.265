...
int MotionEstimate::motionEstimate(ReferencePlanes *ref, // 运动估计主入口:返回最优MV的SATD代价,输出整像素+子像素MV outQMv
                                   const MV &       mvmin,
                                   const MV &       mvmax,
                                   const MV &       qmvp, // 量化的MVP(1/4像素)
                                   int              numCandidates,
                                   const MV *       mvc, // MV候选(供起跳与剪枝)
                                   int              merange, // 搜索范围
                                   MV &             outQMv, // 输出最优1/4像素MV
                                   uint32_t         maxSlices,
                                   pixel *          srcReferencePlane)
{
    ALIGN_VAR_16(int, costs[16]);
    bool hme = srcReferencePlane && srcReferencePlane == ref->fpelLowerResPlane[0]; // 是否HME低分辨率参考
    if (ctuAddr >= 0)
        blockOffset = ref->reconPic->getLumaAddr(ctuAddr, absPartIdx) - ref->reconPic->getLumaAddr(0);
    intptr_t stride = hme ? ref->lumaStride / 2 : ref->lumaStride;
    pixel* fenc = fencPUYuv.m_buf[0]; // 原始像素块
    pixel* fref = srcReferencePlane == 0 ? ref->fpelPlane[0] + blockOffset : srcReferencePlane + blockOffset; // 参考像素基址

    setMVP(qmvp); // 设置MVP用于MVD比特代价计算

    MV qmvmin = mvmin.toQPel(); // 搜索范围转1/4像素
    MV qmvmax = mvmax.toQPel();
     ...
     * (mode + MVD bits). */
    // 整像素搜索用SAD+MVD代价;子像素用SATD+MVD代价

    // measure SAD cost at clipped QPEL MVP
    MV pmv = qmvp.clipped(qmvmin, qmvmax); // MVP裁剪到搜索范围
    MV bestpre = pmv;
    int bprecost; // MVP代价基准(后续与搜索结果比较,防止搜索反而劣化)

    if (ref->isLowres)
        bprecost = ref->lowresQPelCost(fenc, blockOffset, pmv, sad, hme);
    else
        bprecost = subpelCompare(ref, pmv, sad); // 在MVP处测SAD代价

    /* re-measure full pel rounded MVP with SAD as search start point */
    MV bmv = pmv.roundToFPel(); // 当前最优MV,从MVP整像素化开始
    int bcost = bprecost;
    if (pmv.isSubpel())
        bcost = sad(fenc, FENC_STRIDE, fref + bmv.x + bmv.y * stride, stride) + mvcost(bmv << 2); // SAD + MVD代价

    // measure SAD cost at MV(0) if MVP is not zero
    if (pmv.notZero()) // 测零MV代价(常见于静止背景)
    {
    ...
    for (int i = 0; i < numCandidates; i++) // 测量各MV候选代价,更新bestpre(最优起跳点)
    {
    ...
    pmv = pmv.roundToFPel();
    MV omv = bmv;  // current search origin or starting point // 当前搜索起点

    int search = ref->isHMELowres ? (hme ? searchMethodL0 : searchMethodL1) : searchMethod;
    switch (search) // 按搜索方法分发:DIA/HEX/UMH/STAR/SEA/FULL
    {
    case X265_DIA_SEARCH: // 菱形搜索,半径1
    {
        /* diamond search, radius 1 */
        bcost <<= 4; // 低4位编码方向,代价左移4位
        int i = merange;
        do
        {
            COST_MV_X4_DIR(0, -1, 0, 1, -1, 0, 1, 0, costs); // 测4个菱形候选点代价
            if ((bmv.y - 1 >= mvmin.y) & (bmv.y - 1 <= mvmax.y))
            ...
            if (!(bcost & 15)) // 最优仍是中心,收敛退出
                break;
            bmv.x -= (bcost << 28) >> 30; // 按方向移动中心
            bmv.y -= (bcost << 30) >> 30;
            bcost &= ~15; // 清除方向位
        }
    ...
    case X265_HEX_SEARCH: // 六边形搜索,半径2
    {
me_hex2:
      ...
      /* equivalent to the above, but eliminates duplicate candidates */
        COST_MV_X3_DIR(-2, 0, -1, 2,  1, 2, costs); // 测六边形上半部3点
        bcost <<= 3; // 低3位编码方向
        ...
        if (bcost & 7) // 找到比中心更优的点,迭代六边形搜索
        {
            int dir = (bcost & 7) - 2;

            if ((bmv.y + hex2[dir + 1].y >= mvmin.y) & (bmv.y + hex2[dir + 1].y <= mvmax.y))
            {
                bmv += hex2[dir + 1]; // 移动到新的六边形中心

                /* half hexagon, not overlapping the previous iteration */
                for (int i = (merange >> 1) - 1; i > 0 && bmv.checkRange(mvmin, mvmax); i--) // 半六边形迭代(只测3个新点)
                {
        ...
        /* square refine */ // 六边形收敛后,做8点方形精化
        int dir = 0;
        COST_MV_X4_DIR(0, -1,  0, 1, -1, 0, 1, 0, costs); // 测十字4点
        if ((bmv.y - 1 >= mvmin.y) & (bmv.y - 1 <= mvmax.y))
            COPY2_IF_LT(bcost, costs[0], dir, 1);
        if ((bmv.y + 1 >= mvmin.y) & (bmv.y + 1 <= mvmax.y))
            COPY2_IF_LT(bcost, costs[1], dir, 2);
        COPY2_IF_LT(bcost, costs[2], dir, 3);
        COPY2_IF_LT(bcost, costs[3], dir, 4);
        COST_MV_X4_DIR(-1, -1, -1, 1, 1, -1, 1, 1, costs); // 测对角4点
        if ((bmv.y - 1 >= mvmin.y) & (bmv.y - 1 <= mvmax.y))
            COPY2_IF_LT(bcost, costs[0], dir, 5);
        if ((bmv.y + 1 >= mvmin.y) & (bmv.y + 1 <= mvmax.y))
            COPY2_IF_LT(bcost, costs[1], dir, 6);
        ...
        if ((bmv.y + 1 >= mvmin.y) & (bmv.y + 1 <= mvmax.y))
            COPY2_IF_LT(bcost, costs[3], dir, 8);
        bmv += square1[dir]; // 移到方形最优点
        break;
    }

    case X265_UMH_SEARCH: // 非对称十字多六边形搜索
    {
        int ucost1, ucost2;
        int16_t cross_start = 1;

        /* refine predictors */ // 先对MVP、零MV、bestmv各做一次菱形精化
        omv = bmv;
        ucost1 = bcost;
        ...
        /* Early Termination */ // 早退:SAD较小则小范围搜索后退出
        omv = bmv;
        ...
        // adaptive search range based on mvc variability // 根据候选MV离散度自适应调整搜索范围
        if (numCandidates)
        {
            ...
            merange = (merange * range_mul[mvd_ctx][sad_ctx]) >> 2; // 按SAD与MVD上下文缩放范围
        }

        /* FIXME if the above DIA2/OCT2/CROSS found a new mv, it has not updated omx/omy.
         * we are still centered on the same place as the DIA2. is this desirable? */
        CROSS(cross_start, merange, merange >> 1); // 非对称十字搜索(水平全宽,垂直半宽)
        COST_MV_X4(-2, -2, -2, 2, 2, -2, 2, 2);

        /* hexagon grid */ // 多层六边形网格搜索
        omv = bmv;
        const uint16_t *p_cost_omvx = m_cost_mvx + omv.x * 4;
        const uint16_t *p_cost_omvy = m_cost_mvy + omv.y * 4;
        uint16_t i = 1;
        do // 由内向外逐层(步长i)16点六边形网格
        {
...
#undef MIN_MV
        ...
        while (++i <= merange >> 2); // 多层六边形到最大范围
        if (bmv.checkRange(mvmin, mvmax))
            goto me_hex2; // 转六边形迭代精化
        break;
    }

    case X265_STAR_SEARCH: // Adapted from HM ME // 星形搜索(HM风格)
    {
        int bPointNr = 0;
        int bDistance = 0;

        const int EarlyExitIters = 3;
        StarPatternSearch(ref, mvmin, mvmax, bmv, bcost, bPointNr, bDistance, EarlyExitIters, merange, hme); // 星形模式搜索(早退3次迭代)
        if (bDistance == 1)
        {
        ...
        if (bDistance > RasterDistance) // 星形命中点过远,先做大步长光栅搜索
        {
        ...
        while (bDistance > 0) // 以当前最优点为中心反复星形搜索直至收敛
        {
    ...
    case X265_SEA: // 旁路:SEA逐步消元算法(积分图+SAD下界剪枝),主线可跳过
    {
    ...
    case X265_FULL_SEARCH: // 全搜索(穷举),用sad_x4加速
    {
    ...
    if (bprecost < bcost) // 搜索结果不如MVP候选,回退到bestpre
    {
        bmv = bestpre;
        bcost = bprecost;
    }
    else
        bmv = bmv.toQPel(); // promote search bmv to qpel // 提升到1/4像素

    const SubpelWorkload& wl = workload[this->subpelRefine]; // 子像素精化工作负载(迭代次数/方向)
    ...
    else if (ref->isLowres) // 旁路:低分辨率子像素精化,主线可跳过
    {
        int bdir = 0;
        for (int i = 1; i <= wl.hpel_dirs; i++)
        {
    ...
    else // 主线:整像素子像素精化(半像素+1/4像素)
    {
        pixelcmp_t hpelcomp;

        if (wl.hpel_satd) // 半像素用SATD还是SAD
        {
            bcost = subpelCompare(ref, bmv, satd) + mvcost(bmv);
            hpelcomp = satd;
        }
        ...
        for (int iter = 0; iter < wl.hpel_iters; iter++) // 半像素精化:步长2个1/4像素
        {
            int bdir = 0;
            for (int i = 1; i <= wl.hpel_dirs; i++)
            {
        ...
        /* if HPEL search used SAD, remeasure with SATD before QPEL */
        if (!wl.hpel_satd) // 半像素用SAD的,进入1/4像素前用SATD重测
            bcost = subpelCompare(ref, bmv, satd) + mvcost(bmv);

        for (int iter = 0; iter < wl.qpel_iters; iter++) // 1/4像素精化:步长1个1/4像素
        {
            int bdir = 0;
            for (int i = 1; i <= wl.qpel_dirs; i++)
            {
    ...
    x265_emms(); // 清MMX状态
    outQMv = bmv; // 输出最优1/4像素MV
    return bcost; // 返回最优SATD代价
}
