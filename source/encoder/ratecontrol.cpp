...
int RateControl::rateControlStart(Frame* curFrame, RateControlEntry* rce, Encoder* enc)
{
    ...
    int startOrdinal = rce->encodeOrder * 2; // 起始序号:每帧占两个序号位(start+end)

    // 帧间同步:等待前一帧 rateControlEnd 完成,保证 RC 状态机按编码顺序推进
    ...
    {
        // 旁路:动态 zone 重配置窗口(非 bResetZoneConfig 模式),主线可跳过
    ...
    {
        // 旁路:Zone 切换(传统 bResetZoneConfig 模式),命中 startFrame 时切换 zone 参数并重置 RC,主线可跳过
    ...
    if (m_param->rc.bStatRead)
    {
        // 2-pass 读取:从第一遍统计中拷贝对应帧的 RCE 数据到当前 rce
    ...
    {
        // 场景切换重置:标记场景过渡,重置预测器,避免沿用旧场景的 QP/复杂度
    ...
    if (m_isVbv)
    {
        // VBV 行级预测器初始化:场景切换后给 3 类×2 行预测器赋默认系数
        ...
        updateVbvPlan(enc); // 更新 VBV 计划(bufferFill/bufferRate)
        rce->bufferFill = m_bufferFill;
        rce->vbvEndAdj = false;
        if (m_param->vbvBufferEnd && rce->encodeOrder >= m_param->vbvEndFrameAdjust * m_param->totalFrames)
        {
            // 接近末尾时启用 VBV 末尾调整,让 buffer 收敛到 vbvBufferEnd
            rce->vbvEndAdj = true;
            rce->targetFill = 0;
        }

        // MaxAUSize 计算:依据 MinCr/Level 限制本帧最大字节数,防止违反 HRD
    ...
    {
        // 旁路:2-pass CRF 模式直接复用第一遍 qScale,不重新估算,主线可跳过
    ...
    {
        // qScale 决策主路径:ABR/2-pass 调用 rateEstimateQscale 得到 q,再 clip 到 [qpMin,qpMax]
        if (m_isAbr || m_isVbv)
        {
            ...
            /* Detect a pattern for B frames with same SATD cost to identify a series of static frames
        ...
        double q = x265_qScale2qp(rateEstimateQscale(curFrame, rce)); // 核心:估算 qScale 并转 QP
        ...
        accumPQpUpdate(); // 累积 P 帧 QP 加权均值(供后续 I 帧及 ABR 重置使用)
    ...
    {
        // 旁路:用户外部强制 QP(如 AQ/cuTree 覆盖),覆盖前面估算结果,主线可跳过
...
double RateControl::rateEstimateQscale(Frame* curFrame, RateControlEntry *rce)
{
    ...
    {
        // 旁路:滑动窗口 SATD 均值(bLiveVBV2pass/ABR),用于误预测保护,主线可跳过
    ...
    if (m_sliceType == B_SLICE)
    {
        // B 帧分支:B 帧无独立 RC,以前后参考帧 QP 按时间距离加权 + 偏移得到
        ...
        double q0 = m_curSlice->m_refFrameList[0][0]->m_encData->m_avgQpRc; // 前向参考 QP
        double q1 = m_curSlice->m_refFrameList[1][0]->m_encData->m_avgQpRc; // 后向参考 QP
        bool i0 = prevRefSlice->m_sliceType == I_SLICE;
        bool i1 = nextRefSlice->m_sliceType == I_SLICE;
        int dt0 = abs(m_curSlice->m_poc - prevRefSlice->m_poc); // 到前向参考的时间距离
        int dt1 = abs(m_curSlice->m_poc - nextRefSlice->m_poc); // 到后向参考的时间距离
        ...
        if (i0 && i1)
            ...
            q = (q0 * dt1 + q1 * dt0) / (dt0 + dt1); // 核心:按时间距离加权前后参考 QP

        if (IS_REFERENCED(curFrame))
            q += m_pbOffset / 2; // 被参考的 B 帧偏移减半
        else
            q += m_pbOffset; // 普通 B 帧加上完整 PB 偏移
        ...
        {
            // 旁路:grain 模式首个 mini-GOP 的 QP 平滑,主线可跳过
        ...
        if (m_isVbv)
        {
            // VBV clip:把 qScale 限制在 P 帧 qScale 的 lstep 邻域内,然后调用 clipQscale 做 VBV 调整
            lmin = m_lastQScaleFor[P_SLICE] / m_lstep;
            lmax = m_lastQScaleFor[P_SLICE] * m_lstep;
            ...
            if (!m_2pass || m_param->bliveVBV2pass)
            {
                ...
                qScale = clipQscale(curFrame, rce, qScale); // 核心:调用 VBV-lookahead 调整
    ...
    {
        // 非 B 帧分支:基于 2-pass 或 1-pass ABR/CRF 估算 qScale
        double abrBuffer = 2 * m_rateTolerance * m_bitrate;
        if (m_2pass)
        {
            // 2-pass:用第一遍 expectedBits 与当前预测位的差异调整 q,并按整体进度做收尾修正
            ...
            if (zone)
            {
                // 旁路:zone 强制 QP 或按 bitrateFactor 缩放,主线可跳过
                if (zone->bForceQp)
            ...
            if(!m_param->bliveVBV2pass)
                q /= x265_clip3(0.5, 2.0, (double)(abrBuffer - diff) / abrBuffer); // ABR 缓冲区偏离修正
            ...
            if (m_isVbv)
            {
                if (!m_param->bliveVBV2pass)
                {
                    // VBV 防溢出:若预测 bufferFill 不足,逐步抬高 q 直到不溢出
                    /* Do not overflow vbv */
                ...
                else
                {
                        ...
                        * mispredictions by Rate Control pass 1 statistics analysis */
                        q = clipQscale(curFrame, rce, q); // liveVBV2pass:用 VBV-lookahead 调整
                }
            }
        ...
        {
            // 1-pass ABR/CRF:基于短期复杂度 + 整体比特偏差估算 q
            /* 1pass ABR */
            ...
            if (m_param->rc.rateControlMode == X265_RC_CRF)
            {
                q = getQScale(rce, m_rateFactorConstant); // CRF:由 rateFactor 直接算 q
                x265_zone* zone = getZone();
                if (zone)
                {
                    if (zone->bForceQp)
            ...
            {
                // ABR:用 wantedBitsWindow/cplxrSum 得到目标 qScale,再叠加 overflow/grain 调整
                if (!m_param->rc.bStatRead)
                ...
                if (m_isGrainEnabled)
                {
                    // 旁路:grain 模式对 qScale 的额外平滑,主线可跳过
            ...
            if ((m_sliceType == I_SLICE && m_param->keyframeMax > 1
                && m_lastNonBPictType != I_SLICE && !m_isAbrReset) || (m_isNextGop && !m_framesDone))
            {
                // 新 GOP 的首个 I 帧:用累积 P QP 推算 I 帧 q,并按 ipFactor 缩放
                if (!m_param->rc.bStrictCbr)
            ...
            else if (m_framesDone > 0)
            {
                if (m_param->rc.rateControlMode != X265_RC_CRF)
                {
                    // 限制 q 在上一同类型帧 qScale 的 lstep 邻域内,再按 overflow 放宽
            ...
            q = clipQscale(curFrame, rce, q); // 核心:调用 VBV-lookahead 调整
        ...
        if ((m_curSlice->m_poc == 0 || m_lastQScaleFor[P_SLICE] < q) && !(m_2pass && !m_isVbv))
            m_lastQScaleFor[P_SLICE] = q * fabs(m_param->rc.ipFactor); // 维护 P 帧 qScale(按 ipFactor 折算)
...
{
    // VBV-lookahead 核心:根据 lookahead 内未来帧的计划类型/SATD,推演 buffer 走势,
    // 反向调整当前 q,使 lookahead 末尾 buffer 落在 [minVbvFullness, maxVbvFullness] 区间
    ...
    if (m_isVbv && m_currentSatd > 0 && curFrame)
    {
        ...
        {
           // 主分支:有 lookahead 时,迭代调整 q 使 lookahead 末尾 buffer 合理
            ...
            * is in a reasonable state by the end of the lookahead. */
            ...
            for (int iterations = 0; iterations < 1000 && loopTerminate != 3; iterations++)
            {
                double frameQ[3];
                double curBits;
                curBits = predictSize(&m_pred[m_predType], q, (double)m_currentSatd); // 当前帧预测 bits
                double bufferFillCur = m_bufferFill - curBits; // 假设编完当前帧后的 buffer
                double targetFill;
                double totalDuration = m_frameDuration;
                // 由当前 q 推算 I/P/B 三类帧的等效 q(按 ipFactor/pbFactor 折算)
                ...
                // 遍历 lookahead 中未来帧,逐帧累计 buffer 增减,直到累计时长达 1.0 秒或耗尽计划
                bool iter = true;
                for (int j = 0; bufferFillCur >= 0 && iter ; j++)
                {
                    ...
                    double wantedFrameSize = m_vbvMaxRate * m_frameDuration; // 每帧流入 buffer 的比特
                    ...
                    curBits = predictSize(&m_pred[predType], frameQ[type], (double)satd); // 未来帧预测 bits
                ...
                if (rce->vbvEndAdj)
                {
                    // 末尾调整分支:让 buffer 平滑收敛到 vbvBufferEnd
                ...
                else
                {
                    ...
                    // 普通分支:目标是 lookahead 末尾 buffer ≥ minVbvFullness
                    double finalDur = 1;
                    if (m_param->rc.bStrictCbr)
                    {
            ...
            q = X265_MAX(q0 / 2, q); // lookahead 调整最多把 q 降到原值一半
        }
        else
        {
            // 无 lookahead 的回退分支:纯反应式 VBV,直接按当前 bufferFill 调 q
        ...
        // MinCR 限制:若预测 bits 超过 frameSizeMaximum,按比例抬 q
        ...
        // 误预测保护:若当前 SATD 高于滑动均值却把 q 降了一半,则回退到 q0
        ...
        {
            // CRF max 增量限制:不允许 q 比 qpNoVbv + maxIncrement 对应的 q 更低
    ...
    if (!curFrame && m_2pass)
    {
        // 旁路:2-pass 无 curFrame(预填阶段)的 sigmoid 平滑,主线可跳过
        double min = log(lmin);
        double max = log(lmax);
...
{
    // 行级 VBV RC:每编完一行调用,用已编码行的实际 bits 更新行预测器,并按预测总量/缓冲余量微调本行 QP
    FrameData& curEncData = *curFrame->m_encData;
    ...
    updatePredictor(rce->rowPred[0], qScaleVbv, (double)rowSatdCost, encodedBits); // 用本行实际 bits 训练行预测器[0](帧间预测)
    if (curEncData.m_slice->m_sliceType != I_SLICE && !m_param->rc.bEnableConstVbv)
    {
        // 非 I 帧:若 QP 比参考帧对应行低,额外用 intra SATD 更新预测器[1](帧内回退预测)
        Frame* refFrame = curEncData.m_slice->m_refFrameList[0][0];
    ...
    if (m_rateFactorMaxIncrement)
        qpAbsoluteMax = X265_MIN(qpAbsoluteMax, rce->qpNoVbv + m_rateFactorMaxIncrement); // CRF max 限制

    if (m_rateFactorMaxDecrement)
        qpAbsoluteMin = X265_MAX(qpAbsoluteMin, rce->qpNoVbv - m_rateFactorMaxDecrement); // CRF min 限制

    double qpMax = X265_MIN(prevRowQp + m_param->rc.qpStep, qpAbsoluteMax); // 单行 QP 上限:不超过上一行 + qpStep
    double qpMin = X265_MAX(prevRowQp - m_param->rc.qpStep, qpAbsoluteMin); // 单行 QP 下限:不低于上一行 - qpStep
    double stepSize = 0.5;
    ...
    {
        // 非末行:基于预测累计 bits 与计划/缓冲余量做 QP 微调(核心调整逻辑)
        /* More threads means we have to be more cautious in letting ratecontrol use up extra bits. */
        double rcTol = bufferLeftPlanned / m_param->frameNumThreads * m_rateTolerance;
        int32_t encodedBitsSoFar = 0;
        double accFrameBits = predictRowsSizeSum(curFrame, rce, qpVbv, encodedBitsSoFar); // 预测全帧总 bits
        ...
        if (encodedBitsSoFar < 0.05f * rce->frameSizePlanned)
            qpMax = qpAbsoluteMax = prevRowQp; // 已编 bits 过少时不允许抬 QP,避免被顶部平坦区误导
        ...
        double abrOvershoot = (accFrameBits + m_totalBits - m_wantedBitsWindow) / totalBitsNeeded; // ABR 超码率比例

        // 抬 QP 循环:预测超计划/缓冲不足/vbvEndAdj 目标不足时,逐步抬 QP
        while (qpVbv < qpMax
        ...
        // 降 QP 循环:预测显著低于计划/缓冲很满/vbvEndAdj 目标超出时,逐步降 QP
        while (qpVbv > qpMin
        ...
        {
            // strict-CBR 收尾阶段(进度>75%)若仍超码率,额外抬 QP;反之若显著欠码率则降 QP
        ...
        // 防 VBV 下溢/MinCr 违规:若预测会触发,持续抬 QP 直到安全
    ...
    else
    {
        ...
        {
            qpVbv = qpMax; // 末行兜底:直接拉到 qpMax 触发重编
            return -1;
        }
    }
    return 0;
...
{
    int orderValue = m_startEndOrder.get();
    int endOrdinal = (rce->encodeOrder + m_param->frameNumThreads) * 2 - 1; // end 序号:留出 frameNumThreads 的并发窗口
    while (orderValue < endOrdinal && !m_bTerminated)
    {
        // 帧间同步:等待前面 frameNumThreads 个帧的 start 完成,保证 end 按序进行
    ...
    if (m_param->rc.aqMode || m_isVbv || m_param->bAQMotion || bEnableDistOffset)
    {
        ...
        {
            // 统计本帧实际行级 avgQpRc(由 VBV 行 RC 决定),回写到 curEncData/rce
            double avgQpRc = 0;
        ...
        {
            // 统计本帧最终 avgQpAq(含 AQ/cutree/失真调整后)
            double avgQpAq = 0;
    ...
    if (m_isAbr)
    {
        if (m_param->rc.rateControlMode == X265_RC_ABR && !m_param->rc.bStatRead)
            checkAndResetABR(rce, true); // ABR 失控检测/重置
    }
    if (m_param->rc.rateControlMode == X265_RC_CRF)
    {
        // 旁路:CRF 模式反向计算等效 rateFactor,主线可跳过
    ...
    {
        // ABR 状态更新:I 帧开销摊销到后续帧,累积 cplxrSum/wantedBitsWindow/totalBits
        ...
        if (!m_param->rc.bStatWrite && !m_param->rc.bStatRead)
        {
            if (rce->sliceType == I_SLICE)
            {
                ...
                bits -= m_residualCost * m_residualFrames; // 摊销后的本帧"等效"bits
            }
            else if (m_residualFrames)
            {
                bits += m_residualCost; // 非 I 帧加上摊销过来的 I 帧残留
                m_residualFrames--;
            }
        }
        if (rce->sliceType != B_SLICE)
        ...
        m_wantedBitsWindow += m_frameDuration * m_bitrate; // 累积目标比特
        m_totalBits += bits - rce->rowTotalBits; // 累积实际比特(扣除行级已计)
    ...
    if (m_2pass)
    {
        // 2-pass:累积期望比特总和,用于 1-pass 估算时的整体进度修正
    ...
    if (m_isVbv)
    {
        // VBV 收尾:updateVbv 用实际 bits 更新 buffer 状态,返回 filler 字节数;同步 HRD 时序
        *filler = updateVbv(actualBits, rce);

        curFrame->m_rcData->bufferFillFinal = m_bufferFillFinal;
        for (int i = 0; i < 4; i++)
        {
            curFrame->m_rcData->coeff[i] = m_pred[i].coeff; // 同步 4 类预测器参数到帧 RC 数据
            curFrame->m_rcData->count[i] = m_pred[i].count;
            curFrame->m_rcData->offset[i] = m_pred[i].offset;
        }
        if (m_param->bEmitHRDSEI)
        {
            // 旁路:HRD SEI 时序计算(cpbInitialAT/cpbFinalAT/dpbOutputTime),主线可跳过
    ...
    m_startEndOrder.incr(); // 解锁 m_startEndOrder,唤醒等待的下一帧 rateControlStart
    return 0;
}
