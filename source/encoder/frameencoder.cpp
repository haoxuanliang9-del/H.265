...
{
    THREAD_NAME("Frame", m_jpId); // 设置线程名,便于调试

    if (m_pool)
    {
        m_pool->setCurrentThreadAffinity(); // 绑定 CPU 亲和度(NUMA 友好)
        ...
        if (!m_jpId) //    仅 jpId==0 的首个 FE 负责为本池所有 worker 分配 TLD  
        {
            ...
            m_tld = new ThreadLocalData[numTLD]; // 分配线程局部数据数组
            for (int i = 0; i < numTLD; i++)
            {
                m_tld[i].analysis.initSearch(*m_param, m_top->m_scalingList);
                m_tld[i].analysis.create(m_tld); // 初始化每个 TLD 的分析上下文
            }
            ...
            for (int i = 0; i < m_pool->m_numProviders; i++)
            {
                if (m_pool->m_jpTable[i]->m_isFrameEncoder) /* ugh; over-allocation and other issues here */
                {
                    ...
                    peer->m_tld = m_tld; // 共享 TLD 指针给同池的其他 FE
                }
            }
        }

        if (m_param->bEnableWavefront)
            m_localTldIdx = -1; // cause exception if used // WPP 模式下 worker 用各自 TLD,FE 自身索引置 -1 防误用
        else
            m_localTldIdx = m_pool->m_numWorkers + m_jpId; // 非 WPP:FE 自用 TLD 索引
    }
    else
    {
        m_tld = new ThreadLocalData; // 单线程模式:仅分配一个 TLD
        m_tld->analysis.initSearch(*m_param, m_top->m_scalingList);
        m_tld->analysis.create(NULL);
        m_localTldIdx = 0;
    }

    m_done.trigger();     /* signal that thread is initialized */ // 通知主线程:初始化完成
    m_enable.wait();      /* Encoder::encode() triggers this event */ // 等待主线程触发开始编码
    ...
    while (m_threadActive) // 事件驱动主循环:反复 压缩帧 -> 通知完成 -> 等待下一帧
    {
        ...
        compressFrame(); // 核心:压缩一帧
        m_done.trigger(); /* FrameEncoder::getEncodedPicture() blocks for this event */ // 通知取码者本帧完成
        m_enable.wait(); // 阻塞等待下一帧使能信号
    }
}
...
void FrameEncoder::compressFrame()
{
    ...
    if (bUseWeightP || bUseWeightB)
    {
        ...
        else
        {
            ...
            if (m_pool && wa.tryBondPeers(*this, 1))
                ...
                wa.waitForExit(); // 异步等待加权分析完成
            else
                weightAnalyse(*slice, *m_frame, *m_param); // 同步执行加权分析
        }
    }
    else
        slice->disableWeights(); // 不启用加权:禁用权重表

    if (m_param->analysisSave && (bUseWeightP || bUseWeightB))
        reuseWP = (WeightParam*)m_frame->m_analysisData.wt;
    // 生成运动参考图像引用(为每个 ref 初始化 mref 加权/非加权)
    int numPredDir = slice->isInterP() ? 1 : slice->isInterB() ? 2 : 0; // 预测方向数:P=1, B=2, I=0
    for (int l = 0; l < numPredDir; l++)
    {
        for (int ref = 0; ref < slice->m_numRefIdx[l]; ref++)
        {
            ...
            if ((bUseWeightP || bUseWeightB) && slice->m_weightPredTable[l][ref][0].wtPresent)
                w = slice->m_weightPredTable[l][ref]; // 取该 ref 的加权参数
            slice->m_refReconPicList[l][ref] = slice->m_refFrameList[l][ref]->m_reconPic; // 绑定重建图像
            m_mref[l][ref].init(slice->m_refReconPicList[l][ref], w, *m_param); // 初始化加权预测引用对象
        }
        if (m_param->analysisSave && (bUseWeightP || bUseWeightB))
        {
            // analysisSave 保存权重表
                ...
                for (int l = 0; l < numPredDir; l++)
                {
                    for (int ref = 0; ref < slice->m_numRefIdx[l]; ref++)
                    {
                        ...
                        while (refpic->m_reconRowFlag[rowIdx].get() == 0)
                            refpic->m_reconRowFlag[rowIdx].waitForChange(0); // 阻塞等待参考帧该行重建完成

                        if ((bUseWeightP || bUseWeightB) && m_mref[l][ref].isWeighted)
                            m_mref[l][ref].applyWeight(rowIdx, m_numRows, sliceEndRow, sliceId); // 应用加权预测到该参考行
                    }
                }
    ...
    if (slice->m_bUseSao)
        encodeSlice(0); // SAO 启用时,此处统一完成各行的最终编码
...
void FrameEncoder::processRow(int row, int threadId)
{
    ...
    if (ATOMIC_DEC(&m_activeWorkerCount) == 0)
        m_stallStartTime = x265_mdate(); // 最后一个 worker 下线:记录停顿起点
