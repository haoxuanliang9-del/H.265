...
{
    PreLookaheadGroup pre(*this); // 预分析任务组
    Lowres* frames[X265_LOOKAHEAD_MAX + X265_BFRAME_MAX + 4];
    Frame*  list[X265_BFRAME_MAX + 4];
    memset(frames, 0, sizeof(frames));
    memset(list, 0, sizeof(list));
    int maxSearch = X265_MIN(m_param->lookaheadDepth, X265_LOOKAHEAD_MAX); // 向前搜索的最大帧数
    maxSearch = X265_MAX(1, maxSearch);

    {
        ScopedLock lock(m_inputLock); // 加锁访问输入队列

        Frame *curFrame = m_inputQueue.first();
        int j;
        for (j = 0; j < m_param->bframes + 2; j++) // 取出本批待决策的帧到list(最多B帧数+2)
        {
            if (!curFrame) break;
            list[j] = curFrame;
        ...
        frames[0] = m_lastNonB; // frames[0]为上一个非B帧,作为参考起点
        for (j = 0; j < maxSearch; j++)
        {
            ...
            if (!curFrame->m_lowresInit)
                pre.m_preframes[pre.m_jobTotal++] = curFrame; // 收集尚未做低分辨率初始化的帧,稍后批量预分析
    ...
    if (pre.m_jobTotal) // 对收集到的帧做低分辨率预分析(运动估计、帧代价等)
    {
        if (m_pool)
    ...
    if(m_param->bEnableFades) // 旁路:Fade淡入淡出检测,主线可跳过
    {
         ...
         (m_param->lookaheadDepth && m_param->rc.vbvBufferSize)))
    {
        if(!m_param->rc.bStatRead)
            slicetypeAnalyse(frames, false); // 核心场景切换/B帧自适应决策分析
        ...
        if ((m_param->analysisLoad && m_param->scaleFactor && bIsVbv) || m_param->bliveVBV2pass)
        {
            ...
            vbvLookahead(frames, numFrames, false); // VBV前瞻:为后续帧规划码率
        }
    }
    ...
    if (!m_param->analysisLoad || m_param->bAnalysisType == HEVC_INFO)
    {
        ...
        for (bframes = 0, brefs = 0;; bframes++) // 遍历待定帧,确定每帧slice类型(I/P/B/BREF)
        {
            Lowres& frm = list[bframes]->m_lowres;

            if (frm.sliceType == X265_TYPE_BREF && !m_param->bBPyramid && brefs == m_param->bBPyramid) // B-pyramid不兼容的BREF降级为B
            {
             ...
             * smaller dpb could be supported by smart enough use of mmco, but it's easier just to forbid it. */
                ...
                m_param->maxNumReferences <= (brefs + 3)) // DPB不足时BREF降级为B
            {
                frm.sliceType = X265_TYPE_B;
            ...
            if (((!m_param->bIntraRefresh || frm.frameNum == 0) && frm.frameNum - m_lastKeyframe >= m_param->keyframeMax && // 达到关键帧间隔,强制IDR/I
            ...
            if (frm.bIsFadeEnd){ // Fade结束处强制关键帧
                frm.sliceType = m_param->bOpenGOP && m_lastKeyframe >= 0 ? X265_TYPE_I : X265_TYPE_IDR;
            }
            if (m_param->bResetZoneConfig) // 旁路:RC区域配置重置,主线可跳过
            {
            ...
            {
                if (m_param->bOpenGOP) // openGOP下I帧即可作关键帧
                {
            ...
            if (frm.sliceType == X265_TYPE_IDR && frm.bScenecut && isClosedGopRadl) // 旁路:closedGOP RADL场景切换处理,主线可跳过
            {
            ...
            {
                /* Closed GOP */
                m_lastKeyframe = frm.frameNum; // 更新最近关键帧位置
                frm.bKeyframe = true;
                int zoneRadl = 0;
                if (m_param->bResetZoneConfig) // 旁路:zone配置下的RADL调整,主线可跳过
                {
                    for (int i = 0; i < m_param->rc.zonefileCount; i++)
                    {
                ...
                if (bframes > 0 && !m_param->radl && !zoneRadl) // IDR前一个B帧改为P帧,构成closed GOP
                {
                    list[bframes - 1]->m_lowres.sliceType = X265_TYPE_P;
                    bframes--;
                }
            }
            if (bframes == m_param->bframes || !list[bframes + 1]) // 达到最大B帧数或无更多帧,结束本mini-GOP
            {
                ...
                if (frm.sliceType == X265_TYPE_AUTO || IS_X265_TYPE_B(frm.sliceType))
                    frm.sliceType = X265_TYPE_P; // GOP结尾帧强制为P
            }
            ...
            if (frm.sliceType == X265_TYPE_AUTO)
                ...
                break; // 遇到非B帧(I/P),结束本mini-GOP循环
        }
    }
    else
    {
        for (bframes = 0, brefs = 0;; bframes++) // 旁路:analysisLoad(分析复用)分支,主线可跳过
        {
            Lowres& frm = list[bframes]->m_lowres;
            if (frm.sliceType == X265_TYPE_BREF)
                brefs++;
    ...
    if (bframes)
        list[bframes - 1]->m_lowres.bLastMiniGopBFrame = true; // 标记mini-GOP最后一个B帧
    list[bframes]->m_lowres.leadingBframes = bframes;
    m_lastNonB = &list[bframes]->m_lowres; // 更新上一个非B帧
    m_histogram[bframes]++;

    /* insert a bref into the sequence */
    if (m_param->bBPyramid && bframes > 1 && !brefs) // B-pyramid:在B帧序列中插入一个BREF
    {
    ...
    if (m_param->rc.rateControlMode != X265_RC_CQP) // 非CQP模式需预估帧代价供码率控制使用
    {
        int p0, p1, b;
        ...
        if (!maxSearch)
        {
            for (int i = 0; i <= bframes; i++)
               frames[i + 1] = &list[i]->m_lowres; // 零延迟场景补充frames数组
        }

        /* estimate new non-B cost */
        p1 = b = bframes + 1;
        p0 = (IS_X265_TYPE_I(frames[bframes + 1]->sliceType)) ? b : 0; // I帧p0=p1,否则参考上一个非B帧

        CostEstimateGroup estGroup(*this, frames);

        estGroup.singleCost(p0, p1, b); // 估算非B帧(P/I)的代价
        ...
        if (bframes)
        {
            ...
            for (b = 1; b <= bframes; b++)
            {
                ...
                estGroup.singleCost(p0, p1, b); // 估算各B/BREF帧代价
    ...
    for (int i = 0; i <= bframes; i++) // 从输入队列出队本批帧,记录PTS用于重排序
    {
        Frame *curFrame;
    ...
    int idx = 0;
    list[bframes]->m_reorderedPts = pts[idx++]; // 先输出非B帧(P/I),编码顺序
    ...
    for (int i = 0; i < bframes; i++) // 最后输出普通B帧
    {
    ...
    if (isKeyFrameAnalyse && IS_X265_TYPE_I(m_lastNonB->sliceType)) // 关键帧处再做一次前瞻分析(cuTree/VBV)
    {
        m_inputLock.acquire();
        Frame *curFrame = m_inputQueue.first();
        frames[0] = m_lastNonB;
        ...
        frames[j + 1] = NULL;
        if (!m_param->rc.bStatRead)
            slicetypeAnalyse(frames, true); // 关键帧场景:再做slicetypeAnalyse(keyframe=true)
        bool bIsVbv = m_param->rc.vbvBufferSize > 0 && m_param->rc.vbvMaxBitrate > 0;
        if ((m_param->analysisLoad && m_param->scaleFactor && bIsVbv) || m_param->bliveVBV2pass)
        {
            int numFrames;
            ...
            vbvLookahead(frames, numFrames, true); // 关键帧场景VBV前瞻
        }
    }
    m_outputLock.release();
}
