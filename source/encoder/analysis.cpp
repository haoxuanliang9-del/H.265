...
Mode& Analysis::compressCTU(CUData& ctu, Frame& frame, const CUGeom& cuGeom, const Entropy& initialContext)
{
    m_slice = ctu.m_slice;
    m_frame = &frame;
    m_bChromaSa8d = m_param->rdLevel >= 3; // RD≥3 时启用色度 SA8D
    ...
    // QP 设置:支持 DQP 时按 CU 尺寸计算,否则用 slice 级 QP,并由 QP 推导 lambda
    ...
    // 上下文加载:载入初始熵编码上下文,并拷贝原始像素到深度0的 fenc
    ...
    // 旁路:analysisMultiPassRefine 多遍 refine 复用 MV/Ref,主线可跳过
    ...
    // 旁路:analysisSave/Load 复用分析数据,主线可跳过
    ...
    // 主线分发:按 slice 类型(I_SLICE / P/B_SLICE)走不同压缩路径
    if (m_slice->m_sliceType == I_SLICE)
    {
        ...
        compressIntraCU(ctu, cuGeom, qp); // I_SLICE 主路径:帧内 CU 压缩
    }
    else
    {
        ...
        // 旁路:AVC_INFO / loadReuseLevel==10 时直接拷贝分析结果,主线可跳过
        if (bCopyAnalysis)
        {
        ...
        if (m_param->bIntraRefresh && m_slice->m_sliceType == P_SLICE &&
            ...
            compressIntraCU(ctu, cuGeom, qp); // P_SLICE 帧内刷新列:强制走帧内
        else if (!m_param->rdLevel)
        {
            ...
            compressInterCU_rd0_4(ctu, cuGeom, qp); // rdLevel==0 主路径:低复杂度帧间搜索

            /* generate residual for entire CTU at once and copy to reconPic */
            encodeResidue(ctu, cuGeom); // 整 CTU 统一编码残差并写回 reconPic
        }
                ...
                ((m_param->bAnalysisType == AVC_INFO) && m_param->analysisLoadReuseLevel >= 7 && ctu.m_numPartitions <= 16))
        {
            ...
            compressInterCU_rd0_4(ctu, cuGeom, qp); // rdLevel 1~4 主路径:帧间 RD 搜索
        else
            compressInterCU_rd5_6(ctu, cuGeom, qp); // rdLevel 5~6 主路径:高复杂度帧间 RDO
    }

    if (m_param->bEnableRdRefine || m_param->bOptCUDeltaQP)
        qprdRefine(ctu, cuGeom, qp, qp); // 可选:QP 自适应 RD 细化

    if (m_param->csvLogLevel >= 2)
        collectPUStatistics(ctu, cuGeom);

    return *m_modeDepth[0].bestMode; // 返回 CTU 根节点的最佳模式
}
...
uint64_t Analysis::compressIntraCU(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp)
{
    ...
    // 已决策复用:外部已给定 intra 模式且无需 refine 时,bAlreadyDecided 为真
    ...
    else if (cuGeom.log2CUSize != MAX_LOG2_CU_SIZE && mightNotSplit)
    {
        ...
        // 最小 CU(8x8)且 TU 允许更小时,额外尝试 NxN 四分块
    ...
    if (mightSplit)
    {
        ...
        // 四子块递归:对 4 个子 CU 依次调用 compressIntraCU
        for (uint32_t subPartIdx = 0; subPartIdx < 4; subPartIdx++)
        {
            ...
            if (childGeom.flags & CUGeom::PRESENT)
            {
                ...
                {
                    // 剪枝:bEnableSplitRdSkip 累计子代价,超过当前最优即提前 break
        ...
        if (!skipSplitCheck)
        {
            ...
            checkBestMode(*splitPred, depth); // 与不分裂模式对比,选出当前深度最优
        }
    }
    ...
    /* Copy best data to encData CTU and recon */
    md.bestMode->cu.copyToPic(depth);
    // 回写 reconPic:将重建像素写回,供后续 CU 作参考
...
{
    // VBV 受影响且 QP 偏高时降级到 rd5_6 走完整 RDO
    ...
    if (bHEVCBlockAnalysis || bRefineAVCAnalysis || bNooffloading)
    {
        ...
        // 旁路:m_evaluateInter 二次 refine 路径,主线可跳过
        if (m_evaluateInter)
        {
            if (m_refineLevel == 2)
            {
        ...
        // 旁路:bCTUInfo 外部 CTU 信息复用 merge/skip,主线可跳过
        ...
        // 旁路:analysisLoadReuseLevel 复用前次分析(Skip/Merge),主线可跳过
        ...
        // 旁路:analysisMultiPassRefine 多遍 refine 复用,主线可跳过
        ...
        // 主模式搜索①:SKIP/MERGE 候选评估,得到早期跳过机会
        ...
        if (mightSplit && !skipRecursion)
        {
            ...
            // 4 子块递归:逐个调用 compressInterCU_rd0_4
            for (uint32_t subPartIdx = 0; subPartIdx < 4; subPartIdx++)
            {
                const CUGeom& childGeom = *(&cuGeom + cuGeom.childOffset + subPartIdx);
                if (childGeom.flags & CUGeom::PRESENT)
        ...
        // 主模式搜索③:INTER 2Nx2N / BIDIR / 矩形/非对称 AMP / INTRA,本层不分裂时进入
        if (mightNotSplit && (depth >= minDepth || (m_param->bCTUInfo && !md.bestMode)))
        {
            ...
            if (!skipModes)
            {
                ...
                checkInter_rd0_4(md.pred[PRED_2Nx2N], cuGeom, SIZE_2Nx2N, refMasks); // INTER 2Nx2N 整块运动搜索
                ...
                {
                    md.pred[PRED_BIDIR].cu.initSubCU(parentCTU, cuGeom, qp);
                    checkBidir2Nx2N(md.pred[PRED_2Nx2N], md.pred[PRED_BIDIR], cuGeom); // B_SLICE 双向预测候选
                }

                Mode *bestInter = &md.pred[PRED_2Nx2N];
                if (!skipRectAmp)
                {
                    // 矩形分割 2NxN / Nx2N:基于子块代价阈值决定是否尝试
                    if (m_param->bEnableRectInter)
                    {
                ...
                // 主模式搜索④:RD≥3 时对最优 inter 做完整 RDO,并尝试 INTRA 候选
                if (m_param->rdLevel >= 3)
                {
                    ...
                    if (!chooseMerge)
                    {
                        encodeResAndCalcRdInterCU(*bestInter, cuGeom); // 编码残差并计算 inter 完整 RD 代价
                        checkBestMode(*bestInter, depth);
                    ...
                    // INTRA 候选:仅当 inter 有残差或代价极大时尝试帧内
...
{
    // 结构概览:本函数评估 2Nx2N 的 SKIP/MERGE 候选,挑选代价最低的候选作为本深度最佳模式
    //   1) 生成 merge 候选列表(MV/方向/参考帧)
    //   2) 对每个候选做运动补偿,计算 SA8D 代价
    //   3) rdLevel>0 时再对最优候选编码残差,SKIP vs MERGE 二选一
    //   4) 写回 md.bestMode 并广播 MV/Ref 数据
    uint32_t depth = cuGeom.depth;
    ModeDepth& md = m_modeDepth[depth];
    ...
    MVField candMvField[MRG_MAX_NUM_CANDS][2]; // double length for mv of both lists
    uint8_t candDir[MRG_MAX_NUM_CANDS];
    uint32_t numMergeCand = tempPred->cu.getInterMergeCandidates(0, 0, candMvField, candDir); // 生成 merge 候选列表
    ...
    // 遍历每个 merge 候选:边界合法性过滤 → 运动补偿 → SA8D → 选最小代价
    ...
    if (bestSadCand < 0)
        return; // 无合法候选,md.bestMode 保持 NULL
    ...
    // rdLevel>0:分别评估 SKIP(无残差)与 MERGE(有残差)的 RD,选较优者
    if (m_param->rdLevel)
    {
        ...
        encodeResAndCalcRdInterCU(*tempPred, cuGeom); // 带残差 MERGE 的完整 RD

        md.bestMode = tempPred->rdCost < bestPred->rdCost ? tempPred : bestPred; // SKIP vs MERGE 取较优
    }
    else
        md.bestMode = bestPred; // rdLevel==0:直接用 SA8D 最优候选
...
void Analysis::checkInter_rd0_4(Mode& interMode, const CUGeom& cuGeom, PartSize partSize, uint32_t refMask[2])
{
    ...
    // 旁路:analysisLoadReuseLevel 复用前次的参考帧索引,主线可跳过
    ...
    // 旁路:analysisMultiPassRefine 复用 MV/MVP/Ref,主线可跳过
    if (m_param->analysisMultiPassRefine && m_param->rc.bStatRead && m_reuseInterDataCTU)
    {
        uint32_t numPU = interMode.cu.getNumPartInter(0);
        for (uint32_t part = 0; part < numPU; part++)
    ...
    // 主线:对当前 PU 执行整像素+分像素运动估计与模式选择,结果写入 bestME / predYuv
    ...
    // SA8D 代价计算:亮度失真 + 可选色度失真,折算成 sa8dCost
    ...
    // 旁路:analysisSaveReuseLevel 回写参考帧索引供后续复用,主线可跳过
...
{
    // 结构概览:整 CTU 的残差编码(RD0 路径统一调用),递归到叶节点 CU 后:
    //   - 帧内:执行 intra 残差变换量化 + 色度
    //   - 帧间:计算 pred-fenc 残差 → 变换量化 → 反变换重建写回 reconPic
    //   - merge+2Nx2N 且无 cbf 时降级为 SKIP
    if (cuGeom.depth < ctu.m_cuDepth[cuGeom.absPartIdx] && cuGeom.depth < ctu.m_encData->m_param->maxCUDepth)
    {
        // 当前深度小于决策深度:递归到 4 个子块
        for (uint32_t subPartIdx = 0; subPartIdx < 4; subPartIdx++)
        {
            const CUGeom& childGeom = *(&cuGeom + cuGeom.childOffset + subPartIdx);
            if (childGeom.flags & CUGeom::PRESENT)
    ...
    else // if (cu.isInter(0))
    {
        ...
        residualTransformQuantInter(*bestMode, cuGeom, 0, 0, tuDepthRange); // 帧间变换+量化+反变换

        // merge+2Nx2N 且无残差(cbf=0):降级为 SKIP 模式
        ...
        // 重建写回 reconPic:有 cbf 则 pred+resi,否则直接 copy pred
        if (cu.m_cbf[0][0])
        {
    ...
    cu.updatePic(cuGeom.depth, m_frame->m_fencPic->m_picCsp); // 同步 CU 状态回重建图像
}
