...
void SAO::rdoSaoUnitCu(SAOParam* saoParam, int rowBaseAddr, int idxX, int addr)
{
    Slice* slice = m_frame->m_encData->m_slice;
    const CUData* cu = m_frame->m_encData->getPicCTU(addr);
    int qp = cu->m_qp[0]; // 取当前CTU的亮度QP
    int64_t lambda[2] = { 0 };

    int qpCb = qp + slice->m_pps->chromaQpOffset[0] + slice->m_chromaQpOffset[0]; // 计算色度Cb的QP
    ...
    lambda[0] = (int64_t)floor(256.0 * x265_lambda2_tab[qp]); // 亮度lambda(用于RDO代价加权)
    lambda[1] = (int64_t)floor(256.0 * x265_lambda2_tab[qpCb]); // Use Cb QP for SAO chroma

    const bool allowMerge[2] = {(idxX != 0), (rowBaseAddr != 0)}; // left, up // 是否允许向左/上邻居merge

    const int addrMerge[2] = {(idxX ? addr - 1 : -1), (rowBaseAddr ? addr - m_numCuInWidth : -1)};// left, up // 左/上邻居CTU地址
    ...
    if (m_param->bSaoNonDeblocked)
    {
        memcpy(m_count, m_countPreDblk[addr], sizeof(m_count)); // 旁路:使用去块前像素统计,主线可跳过
        memcpy(m_offsetOrg, m_offsetOrgPreDblk[addr], sizeof(m_offsetOrg));
    }
    else
    {
        memset(m_count, 0, sizeof(m_count)); // 清零像素统计,后续用去块后像素重新统计
        memset(m_offsetOrg, 0, sizeof(m_offsetOrg));
    }

    for (int i = 0; i < planes; i++)
        saoParam->ctuParam[i][addr].reset(); // 复位当前CTU各平面的SAO参数
    // SAO distortion calculation
    m_entropyCoder.load(m_rdContexts.cur); // 加载熵编码上下文,用于估算merge标志的码率
    ...
    memset(m_offset, 0, sizeof(m_offset));
    int64_t bestCost = 0; // 当前CTU的最优SAO代价(初始为0,后续被新参数代价覆盖)
    int64_t rateDist = 0;

    bool bAboveLeftAvail = true; // 标记左/上邻居是否均未启用SAO(typeIdx==-1)
    ...
    bool bSaoOff = (slice->m_sliceType == B_SLICE) && (cu->isSkipped(0) || bAboveLeftAvail); // B帧skip或邻居均SAO关闭时本CTU跳过SAO
    ...
    if (saoParam->bSaoFlag[0])
    {
        ...
        {
            calcSaoStatsCTU(addr, 0); // 统计亮度像素的SAO分类直方图与原始偏移和
            saoStatsInitialOffset(addr, 0);
            saoLumaComponentParamDist(saoParam, addr, rateDist, lambda, bestCost); // 亮度4种SAO类型(Band/Edge0-2/Edge1/Edge2)的RDO择优
        }
    }
    ...
    if (saoParam->bSaoFlag[1])
    {
        ...
        {
            calcSaoStatsCTU(addr, 1); // 统计Cb像素
            calcSaoStatsCTU(addr, 2); // 统计Cr像素
            saoStatsInitialOffset(addr, 1);
            saoChromaComponentParamDist(saoParam, addr, rateDist, lambda, bestCost); // 色度4种SAO类型的RDO择优(仅当亮度已选有效类型)
        }
    }
    if (saoParam->bSaoFlag[0] || saoParam->bSaoFlag[1])
    {
        // Cost of merge left or Up
        for (int mergeIdx = 0; mergeIdx < 2; ++mergeIdx) // 分别尝试向左、向上merge
        {
            if (!allowMerge[mergeIdx])
                continue;
            ...
            for (int plane = 0; plane < planes; plane++)
            {
                ...
                if (typeIdx >= 0)
                {
                    ...
                    for (int classIdx = 0; classIdx < SAO_NUM_OFFSET; classIdx++)
                    {
                        ...
                        estDist += estSaoDist(m_count[plane][typeIdx][classIdx + bandPos], mergeOffset, m_offsetOrg[plane][typeIdx][classIdx + bandPos]); // 用本CTU统计评估邻居offset的失真
                    }
                }
            ...
            int64_t mergeCost = mergeDist + estRate; // merge总代价=加权失真+码率
            if (mergeCost < bestCost) // merge代价优于新参数代价,则采用merge模式
            {
                ...
                for (int plane = 0; plane < planes; plane++)
                {
                    if (saoParam->bSaoFlag[plane > 0])
                    {
                        ...
                        SaoCtuParam* mergeSrcParam = &(saoParam->ctuParam[plane][addrMerge[mergeIdx]]);
                        dstCtuParam->mergeMode = mergeMode; // 复制邻居的SAO参数到本CTU
        ...
        if (saoParam->ctuParam[0][addr].typeIdx < 0)
            m_numNoSao[0]++; // 亮度最终未启用SAO的CTU计数+1
        if (chroma && saoParam->ctuParam[1][addr].typeIdx < 0)
            m_numNoSao[1]++; // 色度最终未启用SAO的CTU计数+1
        m_entropyCoder.load(m_rdContexts.temp);
        m_entropyCoder.store(m_rdContexts.cur); // 保存最优参数对应的熵编码上下文,供后续CTU作merge基准
    }
}
