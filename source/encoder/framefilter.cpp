...
void FrameFilter::processRow(int row)
{
    ...
    {
        processPostRow(row); // 旁路:去块与SAO均关闭,主线可跳过
        return;
    }
    FrameData& encData = *m_frame->m_encData; // 取当前帧的编码数据
    ...
    {        
        ...
        X265_CHECK(ctu->m_bFirstRowInSlice || m_parallelFilter[row - 1].m_lastDeblocked.get() == m_numCols, "previous row not finish"); // 确保上一行去块已完成,避免线程竞争

        m_parallelFilter[row].m_allowedCol.set(m_numCols); // 允许本行所有列参与去块
        m_parallelFilter[row].processTasks(-1); // 投递本行去块任务(处理左侧块的去块)
        ...
        if (ctu->m_bLastRowInSlice)
        {
            ...
            if (m_useSao)
            {
                for(int col = 0; col < m_numCols; col++)
                {
                    ...
                    m_parallelFilter[row].processSaoCTU(saoParam, col); // 对最后一行各CTU做SAO滤波
                }
            }
            ...
            for(int col = 0; col < m_numCols; col++)
            {
                ...
                m_parallelFilter[row].processPostCu(col); // 最后一行边界扩展及重建列计数通知
            }
        }
    }
    ...
    if (!ctu->m_bFirstRowInSlice)
        processPostRow(row - 1); // 上一行的收尾处理(通知重建完成等)

    // NOTE: slices parallelism will be execute out-of-order
    int numRowFinished = 0;
    if (m_frame->m_reconRowFlag) // 通过reconRowFlag统计已完成的重建行数
    {
    ...
    if (numRowFinished == m_numRows) // 所有行均完成重建,进入SAO行尾合并阶段
    {
        if (m_useSao)
        {
            ...
            {
                m_parallelFilter[0].m_sao.m_numNoSao[0] += m_parallelFilter[i].m_sao.m_numNoSao[0]; // 汇总各行的亮度未SAO计数
                m_parallelFilter[0].m_sao.m_numNoSao[1] += m_parallelFilter[i].m_sao.m_numNoSao[1]; // 汇总各行的色度未SAO计数
            }

            m_parallelFilter[0].m_sao.rdoSaoUnitRowEnd(saoParam, encData.m_slice->m_sps->numCUsInFrame); // SAO行尾合并:对整帧SAO做最终RDO决定
        }
    }

    if (ctu->m_bLastRowInSlice)
        processPostRow(row); // 最后一行的收尾处理
}
